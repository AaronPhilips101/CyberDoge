...
async def clone(event):
    ...
    if "-r" in inputArgs:
        await event.edit("`Reverting back to my true identity, the person I cloned was an idiot..`")
        if not client.storage.userObj:
            return await event.edit("`You need to clone a profile before reverting dummy!`")
        await updateProfile(client.storage.userObj, reset=True)
        await event.edit("`Feels good to be back.`")
        return
    elif "-d" in inputArgs:
        client.storage.userObj = False
        await event.edit("`The profile backup has been licked clean.`")
        return
    if not client.storage.userObj:
    ...
    await event.edit("`Stealing this idiot hooman's profile..`")
    await updateProfile(userObj)
    await event.edit("`I am you and you are me, now do whatever you want hooman.`")
