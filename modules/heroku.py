...
async def scale_zero(e):
    ...
    await e.edit(f"`Switched off\nTurn on the `[dynos switch](https://dashboard.heroku.com/apps/{app_name}/resources)` to restart CyberDoge`")
...
async def _restart(message):
    ...
    if "-h" in args:
        ...
        app = heroku.apps()[app_name]
        app.restart()
        return await message.edit("`CyberDoge has been updated and the heroku app has been restarted, it should be back online in a few seconds till then maybe have some chimken with bepis hooman.`")
    await message.edit("`CyberDoge has been restarted.\nTry .alive or .ping to check if its alive.`")
...
async def heroku_logs(event):
    if heroku is None:
        return await event.edit("`Can you set the required env vars dummy hooman?`")
