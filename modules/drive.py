...
async def handler(event):
    ...
    if reply:
        if reply.media is not None and "WebPage" not in str(reply.media):
            ...
            try:
                ...
                await event.edit(f"Downloaded file to `{file_path}` in {ms} seconds, now imma go eat chimken.")
        ...
        if os.path.exists(input_str):
            ...
            await event.edit("404: File not found just like your gurlfren!")
            return False
    ...
    if file_path:
        ...
        if use_shared is True:
            ...
            parent = find_folder(drive_service, "CyberDoge")
        try:
...
async def upload_file(service, file_path, file_name, mime_type, parent, event):
    file_metadata = {
        "name": file_name,
        "description": "Uploaded using CyberDoge",
        "mimeType": mime_type,
        "parents": [parent]
    }
