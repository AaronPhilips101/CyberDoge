...
async def handler(event):
    ...
    del_u = 0
    del_status = "`No deleted accounts found, this group is clean just like my plate.`"
    ...
    if not admin and not creator:
        await event.edit("`I don't have ban rights in this group maybe try bribing the admemes with some bepis and chimken!!`")
        return
    ...
    if del_u > 0:
        del_status = f"CyberDoge yeeted **{del_u}** deleted account(s) into outerspace"
    if del_a > 0:
        del_status = f"CyberDoge yeeted **{del_u}** deleted account(s) into outerspace \
